 const { createClient } = supabase;
  ...
  function calculateOverallProgress() {
    const progresses = [60, 35, 80, 10];
    const average = Math.round(progresses.reduce((a, b) => a + b, 0) / progresses.length);
    
    // animate the num
    const element = document.getElementById('overallProgress');
    let current = 0;
    const increment = average / 20;
